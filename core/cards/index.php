...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!therain_csrf_verify(isset($_POST['csrf_token']) ? $_POST['csrf_token'] : null)) {
        ...
        $action = $_POST['card_action'] ?? 'create';
        $cardId = !empty($_POST['card_id']) ? (int) $_POST['card_id'] : 0;
        ...
        if ($action === 'delete') {
            $statement = $connection->prepare('DELETE FROM identity_cards WHERE id = ? AND tenant_id = ?');
            $statement->bind_param('ii', $cardId, $user['tenant_id']); $statement->execute();
            $message = $statement->affected_rows ? 'Card deleted successfully.' : 'Card was not found.';
            $statement->close();
        } elseif ($holderName === '' || !in_array($themeColor, $colors, true)) {
            $error = 'Holder name and a valid card color are required.';
        } elseif ($action === 'update') {
            $statement = $connection->prepare('UPDATE identity_cards SET holder_name = ?, role_label = ?, phone = ?, email = ?, theme_color = ?, updated_at = NOW() WHERE id = ? AND tenant_id = ?');
            $statement->bind_param('sssssii', $holderName, $roleLabel, $phone, $email, $themeColor, $cardId, $user['tenant_id']); $statement->execute();
            $message = $statement->affected_rows >= 0 ? 'Card updated successfully.' : 'Card was not found.';
            $statement->close();
        } else {
...
foreach ($cards as $card) {
    ...
    $cardMarkup .= '<article class="identity-card-pair" data-card-id="' . (int) $card['id'] . '" style="--identity-card-color:' . $escape($card['theme_color']) . '">'
        ...
        . '<div class="identity-card-actions"><button class="identity-card-print" type="button" data-print-card="' . (int) $card['id'] . '"><i class="fas fa-print"></i> Print front &amp; back</button><button class="identity-card-action" type="button" data-edit-card="' . (int) $card['id'] . '"><i class="fas fa-edit"></i> Edit</button><form method="post" onsubmit="return confirm(\'Delete this card?\');">' . therain_csrf_field() . '<input type="hidden" name="card_action" value="delete"><input type="hidden" name="card_id" value="' . (int) $card['id'] . '"><button class="identity-card-action identity-card-action-danger" type="submit"><i class="fas fa-trash"></i> Delete</button></form></div></article>';
}
if ($cardMarkup === '') {
...
$content .= '<section class="dashboard-card-layout"><article class="dashboard-panel"><div class="panel-heading"><div><i class="fas fa-plus-circle"></i><div><h2>Create or edit a card</h2><small>Link it to an existing staff account</small></div></div></div>';
...
$content .= '</select></label><input type="hidden" name="card_action" value="create" data-card-action><input type="hidden" name="card_id" value="0" data-card-id><label>Card type<select name="card_type"><option value="STAFF">Staff card</option><option value="CUSTOMER">Customer card</option></select></label><label>Holder name<input name="holder_name" required data-holder-name></label><label>Role / job title<input name="role_label" value="Staff member" data-role-label></label><div class="identity-form-grid"><label>Phone<input name="phone" data-phone></label><label>Email<input name="email" type="email" data-email></label></div><fieldset><legend>Choose card color</legend><div class="identity-color-options">';
...
$content .= '</div></fieldset><button class="auth-button" type="submit" data-submit-card><i class="fas fa-id-card"></i> Create card</button></form></article><section><div class="identity-card-grid">' . $cardMarkup . '</div></section></section><script>(function(){var form=document.querySelector(".identity-card-form");var staff=document.querySelector("[data-staff-select]");if(staff){staff.addEventListener("change",function(){var o=this.options[this.selectedIndex];document.querySelector("input[name=holder_name]").value=o.dataset.name||"";document.querySelector("input[name=email]").value=o.dataset.email||"";document.querySelector("input[name=phone]").value=o.dataset.phone||"";document.querySelector("input[name=role_label]").value=o.dataset.role||"Staff member";});}Array.prototype.forEach.call(document.querySelectorAll("[data-edit-card]"),function(button){button.addEventListener("click",function(){var card=document.querySelector("[data-card-id=\""+button.dataset.editCard+"\"]");var details=card.querySelector(".identity-card-details");document.querySelector("input[name=card_action]").value="update";document.querySelector("input[name=card_id]").value=button.dataset.editCard;document.querySelector("input[name=holder_name]").value=details.querySelector("h3").textContent.trim();document.querySelector("input[name=role_label]").value=details.querySelector("p").textContent.trim();document.querySelector("input[name=email]").value=details.querySelector("dl dd").textContent.trim();document.querySelector("input[name=phone]").value=details.querySelectorAll("dl dd")[1].textContent.trim();document.querySelector("[data-submit-card]").innerHTML="<i class=\"fas fa-save\"></i> Save card";form.scrollIntoView({behavior:"smooth",block:"start"});});});Array.prototype.forEach.call(document.querySelectorAll("[data-print-card]"),function(button){button.addEventListener("click",function(){Array.prototype.forEach.call(document.querySelectorAll("[data-card-id]"),function(card){card.classList.toggle("identity-card-print-target",card.getAttribute("data-card-id")===button.getAttribute("data-print-card"));});window.print();});});}());</script>';
