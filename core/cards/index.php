...
foreach ($staff as $member) { $name = trim(($member['first_name'] ?? '') . ' ' . ($member['last_name'] ?? '')) ?: $member['email']; $content .= '<option value="' . (int) $member['id'] . '" data-name="' . $escape($name) . '" data-email="' . $escape($member['email']) . '" data-phone="' . $escape($member['phone']) . '">' . $escape($name . ' - ' . $member['email']) . '</option>'; }
$content .= '</select></label><label>Card type<select name="card_type"><option value="STAFF">Staff card</option><option value="CUSTOMER">Customer card</option></select></label><label>Holder name<input name="holder_name" required data-holder-name></label><label>Role / job title<input name="role_label" value="Staff member"></label><div class="identity-form-grid"><label>Phone<input name="phone" data-phone></label><label>Email<input name="email" type="email" data-email></label></div><fieldset><legend>Choose card color</legend><div class="identity-color-options">';
