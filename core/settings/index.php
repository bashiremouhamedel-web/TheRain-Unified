...
foreach ($groups as $group) {
    ...
    foreach ($sections as $key => $definition) {
        ...
        $groupCards .= '<a class="settings-card ' . ($section === $key ? 'is-active' : '') . '" data-setting-card data-setting-search="' . $escape(strtolower($definition[0] . ' ' . $definition[2] . ' ' . $group)) . '" href="?section=' . rawurlencode($key) . '"><span class="settings-card-icon"><i class="' . $escape($definition[1]) . '"></i></span><span><strong>' . $escape($definition[0]) . '</strong><small>' . $escape($definition[2]) . '</small></span><i class="fas fa-arrow-right settings-card-arrow"></i></a>';
    }
