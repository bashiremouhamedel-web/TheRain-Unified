...
- [x] Phase 8 — Runtime stabilization (instability re-confirmed as environmental, not resolved), complete real HTTP integration testing (13/13, first full auth lifecycle over real HTTP), the manufacturerprice/medicine repair (resolved and applied), a Pharmacy compatibility layer + tenant/store bridge (`p_tenant_bridge`), Unified-registration-to-Pharmacy-dashboard identity bridge (first Unified account to reach the real legacy dashboard), 15 pharmacy.*-namespaced permissions seeded (not yet enforced in legacy pages — no per-employee identity there yet), and an honest investigation of dashboard/currency/notifications/search/audit/transaction-architecture/printing/mobile (see docs/PHARMACY-INTEGRATION-REPORT.md for what was built vs. only documented). See docs/PHASE-8-REPORT.md. (Legacy per-employee identity, currency display wiring, notification/search UI, and broad legacy audit logging remain open, folded into Phase 9+.)
- [x] Phase 9 — Shared transaction-state, currency, audit, and Pharmacy identity foundations. Runtime completion was blocked in the delivery environment.
 [ ] Phase 9 — Shared transaction-state, currency, audit, and Pharmacy identity foundations. Transaction/audit code and documentation are present; runtime completion remains blocked until MariaDB is available, while Supermarket pages remain deferred.
