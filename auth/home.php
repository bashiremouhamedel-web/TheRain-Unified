...
$content = '<section class="dashboard-hero"><div><p class="dashboard-kicker">' . therain_dashboard_escape($moduleName) . '</p><h1>Dashboard</h1><p>Welcome back, ' . therain_dashboard_escape($displayName) . '. Here is your pharmacy overview.</p></div><div class="dashboard-date"><i class="far fa-calendar-alt"></i><span>' . date('d M Y') . '</span></div></section>'
  ...
  . ($canSell ? '<a href="../pos.php"><i class="fas fa-cash-register"></i><span>New sale</span><small>Open POS</small></a>' : '')
  . ($canProducts ? '<a href="../manage-products.php"><i class="fas fa-pills"></i><span>Products</span><small>Manage catalog</small></a>' : '')
  . ($canReports ? '<a href="../summary-report.php"><i class="fas fa-chart-bar"></i><span>Reports</span><small>View summaries</small></a>' : '')
